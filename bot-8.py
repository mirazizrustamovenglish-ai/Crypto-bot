...
def detect_consolidation_breakout(ohlc_4h):
    ...
    for lookback in [8, 12, 16, 20, 30]:
        ...
        # Breakout — diapazondan 0.1% chiqsa yetarli
        if last_close > range_high * 1.001:
            return "PUMP 🚀", range_size, vol_ratio, range_high, lookback
        elif last_close < range_low * 0.999:
...
async def scan_coin(symbol, ticker):
    try:
        ...
        # 4H trend breakout yo'nalishi bilan MOS KELISHI SHART EMAS
        # (yumshoq rejim — signal bersin)
