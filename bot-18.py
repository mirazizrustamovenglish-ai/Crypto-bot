...
async def get_all_tickers():
    """Barcha USDT tickerlarini bir so'rovda olish"""
...
async def get_klines(session, symbol: str, granularity="4h", limit=60):
    """OHLCV ma'lumotlari"""
...
async def get_current_price(symbol: str):
    async with aiohttp.ClientSession() as s:
        ...
        if data:
            ...
            return float(d.get("lastPr", 0) or d.get("close", 0) or 0)
    return None

async def fetch_coin_data(symbol: str, ticker: dict = None):
    """Cache bilan OHLC olish"""
    now = datetime.now().timestamp()
    if symbol in CACHE and now - CACHE[symbol].get("ts", 0) < CACHE_TTL:
        return CACHE[symbol]

    async with aiohttp.ClientSession() as s:
        ohlc = await get_klines(s, symbol, "4h", 60)

    if not ohlc:
        return None

    result = {"ticker": ticker, "ohlc": ohlc, "ts": now}
    CACHE[symbol] = result
    return result
...
def detect_consolidation_breakout(ohlc, vol_24h, change_24h):
    """
    Yumshoq shartlar bilan konsolidatsiya breakout:
    - Diapazon: 15% gacha
    - Volume: 1.3x dan yuqori
    - Lookback: 10-30 sham
    """
    if not ohlc or len(ohlc) < 15:
        return None, 0, 0, 0, 0
    ...
    # Volume yetarli emasmi
    if vol_ratio < 1.3:
        return None, 0, 0, 0, 0

    for lookback in [10, 15, 20, 30]:
        if len(ohlc) < lookback + 3:
            continue

        consol = ohlc[-(lookback+3):-3]
        last = ohlc[-1]
        ...
        # Diapazon 15% dan kichik bo'lsin
        if range_size > 15:
            continue
...
# COIN SKAN
# ============================================================
async def scan_coin(symbol: str, ticker: dict):
    """Bitta coin uchun signal tekshirish"""
    try:
        ...
        if price == 0 or vol_24h < 500_000:  # Min $500K volume
            return None

        # Volume spike tez tekshirish
        avg_vol = vol_24h / (abs(change_24h) / 5 + 1) if change_24h else vol_24h * 0.6
        vol_ratio_quick = vol_24h / avg_vol if avg_vol > 0 else 0
        if vol_ratio_quick < 1.3:
            return None

        # OHLC olish
        all_data = await fetch_coin_data(symbol, ticker)
        if not all_data or not all_data["ohlc"]:
            return None
        ...
        breakout_type, range_size, vol_ratio, breakout_level, lookback = \
            detect_consolidation_breakout(ohlc, vol_24h, change_24h)
        ...
        if vol_ratio >= 3.0: score += 3; factors.append(f"⚡{vol_ratio}x Vol")
        elif vol_ratio >= 2.0: score += 2; factors.append(f"⚡{vol_ratio}x Vol")
        else: score += 1; factors.append(f"⚡{vol_ratio}x Vol")

        if (is_long and trend == "bullish") or (not is_long and trend == "bearish"):
            score += 2; factors.append("📈Trend")
        if bos == ("bullish" if is_long else "bearish"):
            score += 2; factors.append("📐BOS")
        if rsi and ((is_long and rsi < 45) or (not is_long and rsi > 55)):
            score += 1; factors.append(f"📊RSI{rsi}")
        if fvg:
            score += 1; factors.append("🔲FVG")

        strength = "🔴 KUCHLI" if score >= 6 else "🟡 O'RTA" if score >= 4 else "🟢 ZAIF"
        ...
        msg = (
            ...
            f"📦 Konsolidatsiya: `{lookback}` sham | `{range_size:.1f}%` diapazon\n"
            f"🔓 Breakout: `${breakout_level:,.6f}`\n"
            f"{trend_e} Trend: `{trend}` | 📊 RSI: `{rsi or 'N/A'}`\n"
            f"📐 BOS: `{bos or 'Yoq'}` | 🔲 FVG: `{fvg or 'Yoq'}`\n\n"
            f"━━━━━━━━━━━━━━━\n"
            f"🎯 *Trade Setup*\n"
            f"• Entry: `${price:,.6f}`\n"
            ...
            f"{tp_text}"
            f"━━━━━━━━━━━━━━━\n"
            f"24h: `{change_24h:+.2f}%` | Hajm: `${vol_24h/1e6:.1f}M`\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("info_"):
        ...
        await query.edit_message_text(
            "✅ *Signallar yoqildi!*\n"
            "⏱ Har 2 daqiqada 300 token tekshiriladi\n"
            "📭 Har 10 daqiqada natija xabari\n"
        ...
        await query.edit_message_text("⏳ Top 100 token tekshirilmoqda...")
        tickers = await get_all_tickers()
        scan_list = list(tickers.keys())[:100]
        found = 0
...
async def periodic_scan(bot):
    ...
    while True:
        ...
        # Har 30 daqiqada tickerlarni yangilash
        if scan_count % 15 == 0:
        ...
        # Volume bo'yicha saralangan barcha coinlar
