...
# ============================================================
def detect_consolidation_breakout(ohlc, change_24h):
    """
    Konsolidatsiyadan chiqishni aniqlash:
    - Diapazon: 15% gacha
    - Lookback: 10-30 sham
    - Breakout: diapazondan 0.2% chiqsa
    """
    if not ohlc or len(ohlc) < 15:
        return None, 0, 0, 0
...
async def scan_coin(symbol: str, ticker: dict):
    ...
    try:
        ...
        price_change_abs = abs(change_24h)
        if price_change_abs < 3.0:
            return None
        ...
        # Konsolidatsiya breakout
        breakout_type, range_size, breakout_level, lookback = \
            detect_consolidation_breakout(ohlc, change_24h)
