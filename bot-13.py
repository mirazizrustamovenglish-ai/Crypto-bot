...
# ============================================================
async def scan_coin(symbol, ticker, min_score=2):
    try:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("info_"):
        ...
        txt = f"✅ {found} signal topildi!" if found else "📭 Hozircha signal topilmadi."
...
async def periodic_scan(bot):
    ...
    while True:
        ...
        for symbol in sorted_symbols:
            try:
                # Avtomatik skan — faqat kuchli signallar (score 5+)
                result = await scan_coin(symbol, tickers.get(symbol, {}), min_score=5)
                if result:
                    found += 1
