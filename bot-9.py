...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. COIN_MAP dan qidirish (btc -> BTCUSDT)
    symbol = COIN_MAP.get(text)

    # 2. Agar topilmasa — to'g'ridan USDT qo'shib sinash (pepe -> PEPEUSDT)
    if not symbol:
        symbol = text.upper() + "USDT"

    msg_obj = await update.message.reply_text("⏳ Tahlil qilinmoqda...")
    try:
        # Avval Binance da bor-yo'qligini tekshirish
        test_price = await get_current_price(symbol)
        if not test_price:
            # BUSD bilan sinash
            symbol_busd = text.upper() + "BUSD"
            test_price = await get_current_price(symbol_busd)
            if test_price:
                symbol = symbol_busd
            else:
                await msg_obj.edit_text(
                    f"❌ *'{text}'* Binance da topilmadi.\n\n"
                    f"To'g'ri yozing:\n`btc` `eth` `sol` `bnb` `doge` `pepe` `shib`",
